...
bool Tester::insert_edge(){
    ...
    //    fleet.dumpTree();
    //    fleet.listShips();
    return fleet.getSize() == fleetSize && fleet.verifyAVL();
}
bool Tester::insert_error(){
    ...
    fleet.insert(badid);
    if (fleet.getSize() != 0 || !fleet.verifyAVL()) {
        return false;
    }
    fleet.insert(dupe1);
    if (fleet.getSize() != 1 || !fleet.verifyAVL()) {
        return false;
    }
    fleet.insert(dupe2);
    if (fleet.getSize() != 1 || !fleet.verifyAVL()) {
...
bool Tester::test_remove(){
    ...
    if (fleet.getSize() != 10 || !fleet.verifyAVL()) {
        return false;
    }
    fleet.listShips();
    fleet.remove(40001);
    cout << endl;
    fleet.listShips();
    if (fleet.getSize() != 9 || fleet.findShip(40001) || !fleet.verifyAVL()) {
        cout << "Removal Failed!" << endl;
        return false;
    }
    fleet.remove(40000);
    cout << endl << endl << "Testing Removal of Edge Case" << endl;
    if(fleet.getSize() != 8 || fleet.findShip(40000) || !fleet.verifyAVL()){
        cout << "Removal Failed!" << endl;
        return false;
    }
    return true;
...
bool Tester::test_remove_edge()
{
    ...
    while (fleet_size > 0) {
        ...
        fleet.remove(ID);
        fleet_size--;
        if (fleet.getSize() != fleet_size || fleet.findShip(ID) || !fleet.verifyAVL()) {
            cout << "Removal Failed!" << endl;
            return false;
        }
    }
...
bool Tester::test_balance(){
    ...
    for (int i = 0; i < fleet_size; i++){
        Ship ship(ID, CARGO, ALIVE);
        fleet.insert(ship);
        if (fleet.checkImbalance(&ship) > 1){
            cout << "Error, Tree not Balanced" << endl;
            return false;
        }
        ID++;
    }
...
//our code
//fleet.setState(19372, LOST);
//fleet.setState(47597, LOST);
//        fleet.setState(20201, LOST);
//        fleet.listShips();
//        cout << endl;
//        fleet.removeLost();
//        fleet.listShips();
